 ...
 * ```
 */
export function useDevPanel(sectionName: string, controls: ControlGroup) {
    ...
    const previousControlsRef = useRef<ControlGroup | undefined>(undefined);
    ...
    useEffect(() => {
        ...
        if (
            hasControlsChanged(controls, previousControlsRef.current) ||
            !sectionExists
        ) {

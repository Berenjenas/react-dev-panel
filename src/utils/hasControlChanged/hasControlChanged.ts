 ...
 */
export function hasControlsChanged(
    current: ControlGroup,
    previous?: ControlGroup
) {
    ...
    for (const key of currentKeys) {
        ...
        if (
            ...
            previousControl.type === "select"
        ) {
            if (
                JSON.stringify(currentControl.options) !==
                JSON.stringify(previousControl.options)
            ) {
                return true;
            }
        }

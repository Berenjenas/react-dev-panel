...
import type { DevPanelStore } from "@/components/DevPanel/types";
...
export const useDevPanelStore = create<DevPanelStore>()(
	persist(
		(set) => ({
			...
			registerSection: (name: string, controls: ControlsGroup) => {
				set((state) => {
					...
					try {
						...
						if (stored) {
							...
							persistedCollapseState = parsed.state?.sectionCollapseState?.[name] ?? false;
						}
					} catch {
		...
		}),
		{
			...
			partialize: (state) => ({
				...
				sectionCollapseState: Object.fromEntries(Object.entries(state.sections).map(([key, section]) => [key, section.isCollapsed])),
			}),
		},
	),
);
...
export const useDevPanelVisible = () => useDevPanelStore((state) => state.isVisible);
export const useDevPanelCollapsed = () => useDevPanelStore((state) => state.isCollapsed);
export const useDevPanelPosition = () => useDevPanelStore((state) => state.position);
export const useDevPanelSections = () => useDevPanelStore((state) => state.sections);
